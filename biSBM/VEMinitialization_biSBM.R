...
initialRho <- function(listOfBeta, data, modelFamily, model){
  ...
  init <- list(beta=lapply(1:nbOfBeta, function(k) listOfBeta[[k]]),
               ...
               nu   = vector("list", nbOfBeta)   # <-- always present; fill below
  ...
  if (modelFamily=='Gamma'){
    ...
    if (model=='Exp'){
      for (k in 1:nbOfBeta){
        ...
        nu <- list() # store the parameters for the alternative distribution
        nu$mean <- matrix( 1, Q1, Q2)
        nu$sd <- matrix(stats::rgamma(N_Q, 1/dataMean, 1), Q1, Q2)
        pi <-  matrix(stats::rbeta(N_Q,2,2), Q1, Q2)
        init$rho[[k]] <- getRho(Q1, Q2, pi, nu0, nu, data, modelFamily)
      }
    }else{ # 'ExpGamma'
      init$nu <- lapply(1:nbOfBeta, function(k) list(mean = matrix(1, Q1, Q2),
                                              sd = matrix(1, Q1, Q2)))
      dataVar <- stats::var(as.numeric(data))
      for (k in 1:nbOfBeta){
        pi <-  matrix(stats::rbeta(N_Q,2,2), Q1, Q2)
        nu0 <- c(1, stats::rexp(1, dataMean))
        m <- rep(-1, N_Q)
        ...
        init$nu[[k]]$mean <- matrix(m^2/v,Q1,Q2)
        init$nu[[k]]$sd <- matrix(m/v, Q1, Q2)
        init$rho[[k]] <- getRho(Q1, Q2, pi, nu0, init$nu[[k]], data, modelFamily)
      }
    }
  }
  return(init)
