...
#'   \item{\code{Exp1}}{compared to \code{Exp}, the null distribution is set to Exp(1)}
...
#'   \item{\code{Exp1Gamma}}{compared to \code{ExpGamma}, the null distribution is set to Exp(1)}
#' }
...
fitNobiSBM <- function(dataMatrix,
                       model = 'Gauss0',
  ...
  ## ---------- helpers ----------
  .cap_cores <- function(n) {
    if (Sys.info()[["sysname"]] == "Windows") {
      1L
    } else {
      if (n > 1) min(c(round(n), parallel::detectCores())) else 1L
    }
  }
  
  .model_family <- function(model) {
    if (model %in% c('Gauss','Gauss0','Gauss01','Gauss0Var1','GaussEqVar','Gauss0EqVar','Gauss2distr','Gauss02distr','GaussAffil')) return('Gauss')
    if (model %in% c('Exp','Exp1','ExpGamma','Exp1Gamma','Gamma')) return('Gamma')
    stop("Unknown model: ", model)
  }
  
  `%||%` <- function(a, b) if (!is.null(a)) a else b
  
  nbCores <- .cap_cores(nbCores)
  doParallelComputing <- (nbCores > 1)
  n1 <- nrow(dataMatrix); n2 <- ncol(dataMatrix)
  modelFamily <- .model_family(model)
  
  ## ---------- branch: NON-EXCLUSIVE (unequal row/col clusters) ----------
  if (!exclusive_Rows_Cols) {
    Q1min <- max(c(1, min(sbmSize$Q1))); Q1max <- max(sbmSize$Q1)
    Q2min <- max(c(1, min(sbmSize$Q2))); Q2max <- max(sbmSize$Q2)
    possibleQvalues <- expand.grid(Q1min:Q1max, Q2min:Q2max)
    M <- nrow(possibleQvalues)
    
    bestSolutionAtQ <- lapply(1:M, function(elem) list(convergence = list(J = -Inf)))
    
    # initial points for each (Q1,Q2)
    init <- lapply(1:M, function(currentQindice){
      Q1 <- possibleQvalues[currentQindice,1]
      Q2 <- possibleQvalues[currentQindice,2]
      init0 <- initialPoints(Q1, Q2, dataMatrix, modelFamily, model, method='kmeans')
      list(beta = init0$beta[[1]], rho = init0$rho[[1]], nu = init0$nu[[1]])
    })
    ListOfbetaRho <- list(beta = lapply(init, `[[`, "beta"),
                          rho  = lapply(init, `[[`, "rho"),
                          nu   = lapply(init, `[[`, "nu"))
    
    if (doParallelComputing) {
      bestSolutionAtQ <- parallel::mclapply(1:M, function(k){
        mainVEM_Q_par(k, ListOfbetaRho, modelFamily, model, dataMatrix, num.iter)
      }, mc.cores = nbCores)
    } else {
      for (s in 1:M) {
        cat('Q1 =', possibleQvalues[s,1], 'Q2 =', possibleQvalues[s,2],
            'Q1max =', Q1max, 'Q2max =', Q2max, '\n')
        currentInitialPoint <-
          if (! model %in% c('ExpGamma','Exp1Gamma')) list(beta = ListOfbetaRho$beta[[s]], rho = ListOfbetaRho$rho[[s]]) else
            list(beta = ListOfbetaRho$beta[[s]], rho = ListOfbetaRho$rho[[s]], nu = ListOfbetaRho$nu[[s]])
        
        currentSolution <- mainVEM_Q(currentInitialPoint, modelFamily, model, dataMatrix, num.iter)
  ...
  ## ---------- branch: EXCLUSIVE (Q1 == Q2) ----------
  if (exclusive_Rows_Cols) {
    Qmin   <- max(c(1, min(sbmSize$Q1)))
    explor <- if (!is.null(sbmSize$explor)) sbmSize$explor else if (!is.null(sbmSize$explore)) sbmSize$explore else 1.5
    Qmax   <- max(sbmSize$Q1)
    QmaxIsFlexible <- FALSE  # preserved behavior
    ...
    bestSolutionAtQ <- lapply(1:(Qmax - Qmin + 1), function(elem) list(convergence = list(J = -Inf)))
    ...
    while (notConverged) {
      ...
      if (nbOfPasses == 1) {
        ListOfbetaRho <- initialPoints(Q1, Q2, dataMatrix, modelFamily, model, method='kmeans')
        if (Q > Qmin) {
          additionalbetaRho <- initialPointsBySplit(bestSolutionAtQ[[currentQindice - 1]]$sbmParam$clusterProba,
                                                    nbOfBeta = round(Q / 2), dataMatrix, modelFamily, model)
          ...
          if (model %in% c('ExpGamma','Exp1Gamma')) {
            ListOfbetaRho$nu <- append(ListOfbetaRho$nu, additionalbetaRho$nu)
          }
        }
      } else {
        if (up) {
          ListOfbetaRho <- initialPointsBySplit(bestSolutionAtQ[[currentQindice - 1]]$sbmParam$clusterProba,
                                                nbOfBeta = max(c(3, round(Q / 2))), dataMatrix, modelFamily, model)
        } else {
          ListOfbetaRho <- initialPointsByMerge(bestSolutionAtQ[[currentQindice + 1]]$sbmParam$clusterProba,
                                                nbOfBeta = max(c(5, round(Q / 2))), dataMatrix, modelFamily, model)
        }
      }
      ...
      if (doParallelComputing) {
        ...
        ListOfJ <- vapply(ListOfSolutions, function(sol) sol$convergence$J, numeric(1))
      ...
      ## evaluate pass & decide next step
      if (nbOfPasses == 1) {
        if (Q < Qmax) {
        ...
        if ((Q > Qmin) & (Q < Qmax)) {
          ...
          bestQInThisPass <- getBestQ(bestSolutionAtQ)
          if (QmaxIsFlexible) {
            if (bestQInThisPass$ICL > bestQ$ICL) {
              if (bestQInThisPass$Q <= round(Qmax / explor)) {
                if (Q == Qmin) {
                  bestQ <- bestQInThisPass
                  notConverged <- (nbOfPasses < maxNbOfPasses)
                }
              ...
              notConverged <- (nbOfPasses < minNbOfPasses)
            }
          } else { # fixed Qmax
            if (bestQInThisPass$ICL > bestQ$ICL) {
              if (Q == Qmin) {
...
mainVEM_Q <- function(init, modelFamily, model, dataMatrix, num.iter = 5){
  ...
  if (modelFamily == 'Gauss'){
    ...
    if(model %in% c('Exp','Exp1')){
      theta.init <- list(pi=matrix(NA, Q1, Q2),
                         nu0=c(1,1),
                         nu=list(mean=matrix(1,Q1,Q2),
                                 sd = matrix(1,Q1,Q2)))
    }
    if(model %in% c('ExpGamma','Exp1Gamma')){
      theta.init <- list(pi=matrix(NA, Q1, Q2),
  ...
  while (sum(convergence==TRUE)==0){
    ...
    # print(mstep$pi)
    VE <- VEstep(VE, mstep, dataMatrix, modelFamily, fix.iter = num.iter)
    # print(mean(VE$rho[1,1,,]))
...
#'
#' @return list of estimated model parameters and a node clustering; like the output of fitNobiSBM()
mainVEM_Q_par <- function(s, ListOfbetaRho, modelFamily, model, dataMatrix, num.iter){
  currentInitialPoint <- if (! model %in% c('ExpGamma','Exp1Gamma')) list(beta=ListOfbetaRho$beta[[s]], rho=ListOfbetaRho$rho[[s]]) else
    list(beta=ListOfbetaRho$beta[[s]], rho=ListOfbetaRho$rho[[s]], nu=ListOfbetaRho$nu[[s]])
  currentSolution <- mainVEM_Q(currentInitialPoint, modelFamily, model, dataMatrix, num.iter)
...
VEstep <- function(VE, mstep, dataMatrix, modelFamily, fix.iter=5){
  ...
  for (q in 1:Q1){
    for (l in 1:Q2){
      rho_numerateur <- modelDensity(dataMatrix, 
                                     c(mstep$nu$mean[q,l],mstep$nu$sd[q,l]), 
                                     modelFamily) * mstep$pi[q,l]  # taille N
      VE$rho[q,l,,] <-  rho_numerateur / (rho_numerateur + modelDensity(dataMatrix, 
                                                                        mstep$nu0, 
                                                                        modelFamily)*(1-mstep$pi[q,l]))
    }
  }
...
Mstep <- function(VE, mstep, model, dataMatrix, modelFamily){
  ...
  numer_1 <- numer_2 <- numer_3 <- numer_4 <- numer_5 <- numer_6 <- 0
  ...
  for (q in 1:Q1){
    for (l in 1:Q2){
      ...
      if (modelFamily=='Gamma'){
        if (s > 0){
          if (model %in% c('Exp','Exp1')){
            mstep$nu$sd[q,l] <- s/sum(kappa_ql*dataMatrix)
          }
          if (model %in% c('ExpGamma','Exp1Gamma')){
            L_ql <- sum(kappa_ql*log(dataMatrix))/s
            M_ql <- sum(kappa_ql*dataMatrix)/s
            fit <- emv_gamma(L_ql, M_ql, c(mstep$nu$mean[q,l], mstep$nu$sd[q,l]))
            mstep$nu$mean[q,l] <- fit[1]  # shape parameter
            mstep$nu$sd[q,l] <- fit[2]    # rate parameter
          }
        }else{ # if no observations: use random values
          mstep$nu$sd[q,l] <- stats::rgamma(1,2.5,0.5)
        }
        numer_2 <- numer_2 + sum(beta_ql_1mrho)
        denom_1 <- denom_1 + sum(beta_ql_1mrho*dataMatrix)
      }
    } # end for l
  }# end for q
  ...
  if (modelFamily=='Gamma'){
    ...
    if (model %in% c('Exp','ExpGamma')){
      mstep$nu0[2] <- if (denom_1>0) numer_2/denom_1 else 1/mean(dataVector[dataVector<stats::median(dataVector)])
    }
  }
  return(mstep)
}
...
#'
#' @param param parameters of the Gamma distribution (shape and rate parameter)
#' @param L weighted mean of log(data)
#' @param M weighted mean of the data
#'
...
ICL_Q <- function(solutionThisRun, model){
  ...
  if (model %in% c('Gauss01','Exp1Gamma')){
    dimH0 <- 0
    dimH1 <- 2*N_Q
  }
  if (model=='Gauss0Var1'){
